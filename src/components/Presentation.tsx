...
interface Props {
  ...
  /** Der Weg zu den Folgen, die dieses Kapitel behandeln. */
...
export default function Presentation({
  ...
  /**
   * Wie viele Folgen dieses Kapitel behandeln. Der Index kommt erst, wenn
   * jemand ein Buch aufschlägt – der Lesemodus soll deshalb nicht warten.
   */
  ...
  useEffect(() => {
    ...
    loadMedia().then((index) => {
      if (live) setMediaCount(episodesForRef(index, book, chapter).length);
    });
    return () => {
      live = false;
    };
        ...
        >
          <div className="sticky top-0 z-10 border-b-4 border-deep bg-paper px-6 py-4">
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              ...
              {onOpenMedia && mediaCount > 0 && (
                <button
                  onClick={() => onOpenMedia(meta.osis, chapter)}
                  className="inline-flex items-center gap-1.5 bg-paper-2 px-3 py-1.5 text-[11px] font-extrabold tracking-wide text-ink transition hover:bg-gold/40"
                >
